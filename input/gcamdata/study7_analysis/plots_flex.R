...
i = 1
data1 = get(load(paste0(folder,'/V1_2050/L202.Flexitarian_population_',i,'.RData')))
data2 = get(load(paste0(folder,'/L202.Flexitarian_population_',i,'_2080_60-80.RData')))
data3 = get(load(paste0(folder,'/L202.Flexitarian_population_',i,'.RData')))
data = bind_rows(data1 %>%
                   mutate(run = 'V1'),
                 data2 %>%
                   mutate(run = '2080_60-80'),
                 data3 %>%
                   mutate(run = '2040_60-80')) %>%
  select(region,year,flex,population,run)
...
ggplot(data %>%
  ...
  geom_line(aes(x = year, y = flex_percentage, color = run)) +
...
ggplot(data %>%
  ...
  facet_wrap(. ~ region, scales = 'free') +
  geom_line(aes(x = year, y = flex, color = run)) +
...
ggplot(data %>%
  ...
  facet_wrap(. ~ region, scales = 'free') +
  geom_line(aes(x = year, y = new_flex, color = run)) +
