...
module_aglu_L201.snr_parameters <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    save(L201.snr_parameters, file = paste0(outputs_path, '/snr/L201.snr_parameters.RData'))
    if(!dir.exists('output')) dir.create('output')
    save(L201.snr_scenarios_names, file = 'output/L201.snr_scenarios_names.RData')
