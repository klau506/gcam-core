...
module_diets_L202.dietary_change <- function(command, ...) {
  ...
  MODULE_INPUTS <-
    c(FILE = "aglu/diets_scenarios",
      FILE = "aglu/diets_plant_percentage_REF")

  if(command == driver.DECLARE_INPUTS) {
    return(MODULE_INPUTS)
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L202.diets_plant_per_intake_ref",
             "L202.diets_plant_per_intake_list")) # TODO: add SNR
    ...
    logit_function = function(data) {
      data = data %>%
        dplyr::mutate(A = sf - ((1+exp(-k*(MODEL_FINAL_BASE_YEAR-x0)))*(sf - si)/(exp(-k*(MODEL_FINAL_BASE_YEAR-x0)) - exp(-k*(MODEL_HALF_CENTURY_YEAR-x0)))),
    ...
    for (i in 1:nrow(diets_scenarios)) {
      ...
      for (j in 1:nrow(combinations)) {
        diets_plant_per_intake_tmp2 <- diets_plant_per_intake_tmp %>%
          dplyr::mutate(x0 = combinations[j,]$x0,
                        k = combinations[j,]$k)
