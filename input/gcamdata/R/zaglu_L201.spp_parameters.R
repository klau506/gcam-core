...
module_aglu_L201.spp_parameters <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L201.spp_parameters_2 %>% tibble::as_tibble() %>%
      ...
      add_precursors("aglu/A_spp_initial_shares") ->
      L201.spp_parameters
    ...
    write.csv(tmp, file = paste0(outputs_path,'L201.spp_scenarios_database_mapping.csv'), row.names = FALSE)
    tmp = tmp %>%
      dplyr::mutate(type = database)
    write.csv(tmp, file = paste0(outputs_path,'L201.spp_scenarios_database_mapping_type.csv'), row.names = FALSE)
    ...
    save(L201.spp_parameters, file = paste0(outputs_path, '/L201.spp_parameters.RData'))
    save(L201.spp_scenarios_names, file = paste0(outputs_path, '/L201.spp_scenarios_names.RData'))
