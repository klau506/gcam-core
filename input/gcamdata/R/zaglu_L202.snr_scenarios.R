...
module_aglu_L202.snr_scenarios <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    if (!file.exists(paste0(outputs_path, '/snr/L202.snr_logisticFun.RData'))) {
      ...
      save(L202.snr_logisticFun, file = paste0(outputs_path, '/snr/L202.snr_logisticFun.RData'))
    } else {
      load(paste0(outputs_path, '/snr/L202.snr_logisticFun.RData'))
    }
